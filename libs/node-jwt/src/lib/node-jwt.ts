...
export class NodeJwt {
    ...
    constructor(header?: IJwtHeader | Record<string, unknown> | string, claims?: IJwtClaims | Record<string, unknown> | string, secretKey?: string ) {
    ...
    public setHeader(header?: IJwtHeader | Record<string, unknown> | string): this {
        if (!header) {
        ...
        } else if ((header as unknown) instanceof JwtHeader) {
            this.header = header;
        }
        return this;
    }

    public setClaims(claims?: IJwtClaims | Record<string, unknown> | string): this {
        if (!claims) {
            this.claims = new JwtClaims({});
        }
        if (typeof claims === 'string') {
            let jwtClaims: string = base64urldecode(claims);
            jwtClaims = JSON.parse(jwtClaims);
            this.claims = new JwtClaims((jwtClaims as any));
        } else if (typeof claims === 'object') {
            this.claims = new JwtClaims((claims as IClaims))
        } else if ((claims as unknown) instanceof JwtClaims) {
            this.claims = claims;
        }
        return this;
    }
    ...
    public static create(algorithm?: Algorithms, payload?: IClaims | Record<string, unknown>, secretKey?: string) {
        const jwtProvider = new NodeJwt({alg: algorithm}, payload, secretKey);
    ...
    public static verify(token: string, secretKey?: string): Error | boolean {
        ...
        const jwt = new NodeJwt(parts[0], parts[1], secretKey);
