...
export type Validation = (value: string) => InputStatus;
...
export const Field: FC<FieldProps> = ({
  ...
  const onBlur = (newVal: string) => {
    if (required && !newVal) {
      setInputMsgs([
        ...
        {
          status: 'error',
          message: `Please provide a valid value for ${label}`,
        },
      ]);
  ...
  const lastMsg: InputStatus | undefined =
    inputMsgs[inputMsgs.length - 1] || undefined;
  ...
  const inputMsgEl = lastMsg ? (
    <small className={getStatusClass(lastMsg)}>{lastMsg.message}</small>
  ) : null;
  ...
  if ((type === 'select' || type === 'multi-select') && options?.length) {
    ...
    throw new Error(
      `<Field> type provided as '${type}' but options prop is empty!`
    );
  }
